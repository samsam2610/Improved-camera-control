...
FrameFilterHandle._fields_ = [('pFilter', C.c_void_p), ('bHasDialog', C.c_int), ('ParameterCount', C.c_int),
...
class TIS_GrabberDLL(object):
    ...
    get_available_framerates.argtypes = (GrabberHandlePtr, C.c_int, C.POINTER(C.c_float))
    ...
    GetImageDescription.argtypes = (
        GrabberHandlePtr, C.POINTER(C.c_long), C.POINTER(C.c_long), C.POINTER(C.c_int), C.POINTER(C.c_int),)
    ...
    GetPropertyValue.argtypes = (GrabberHandlePtr, C.c_char_p, C.c_char_p, C.POINTER(C.c_long),)
    ...
    GetPropertySwitch.argtypes = (GrabberHandlePtr, C.c_char_p, C.c_char_p, C.POINTER(C.c_long),)
    # ############################################################################
    ...
    SetPropertyAbsoluteValue.argtypes = (GrabberHandlePtr, C.c_char_p, C.c_char_p, C.c_float,)
    
    GetPropertyAbsoluteValue = __tisgrabber.IC_GetPropertyAbsoluteValue
    GetPropertyAbsoluteValue.restype = C.c_int
    GetPropertyAbsoluteValue.argtypes = (GrabberHandlePtr, C.c_char_p, C.c_char_p, C.POINTER(C.c_float),)
...
class TIS_CAM(object):
    ...
    def open(self, unique_device_name):
        """ Open a device
            ...
            """
        test = TIS_GrabberDLL.open_device_by_unique_name(self._handle, self.s(unique_device_name))
    ...
    def GetBufferSize(self):
        width, Height, bpp,  = self.GetImageDescription()
    ...
    def GetImage(self):
        ...
        img = np.ndarray(buffer=Bild.contents, dtype=np.uint8, shape=(lHeight, lWidth, iBitsPerPixel))
        return img
    ...
    def GetImageEx(self):
        ...
        img = np.ndarray(buffer=Bild.contents, dtype=pixeltype, shape=(lHeight, lWidth, iBytesPerPixel))
        return img
    ...
    def SetPropertyValue(self, Property, Element, Value):
        error = TIS_GrabberDLL.SetPropertyValue(self._handle, self.s(Property), self.s(Element), Value)
        return error
    
    def GetPropertyValue(self, Property, Element):
        Value = C.c_long()
        error = TIS_GrabberDLL.GetPropertyValue(self._handle, self.s(Property), self.s(Element), Value)
        return Value.value
    ...
    def SetPropertySwitch(self, Property, Element, Value):
        error = TIS_GrabberDLL.SetPropertySwitch(self._handle, self.s(Property), self.s(Element), Value)
        return error
    
    def GetPropertySwitch(self, Property, Element, Value):
        lValue = C.c_long()
        error = TIS_GrabberDLL.GetPropertySwitch(self._handle, self.s(Property), self.s(Element), lValue)
        Value[0] = lValue.value
        return error
    
    def PropertyOnePush(self, Property, Element):
        error = TIS_GrabberDLL.PropertyOnePush(self._handle, self.s(Property), self.s(Element))
        return error
