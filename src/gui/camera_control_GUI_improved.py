...
from tkinter import Entry, Label, Button, StringVar, IntVar, Tk, END, Radiobutton, filedialog, ttk, Frame, Scale, \
    HORIZONTAL, Spinbox, Checkbutton, DoubleVar
...
class CamGUI(object):
    ...
    def create_output_files(self, subject_name='Sam'):
        ...
        for i in range(len(self.cam)):
            ...
            self.ts_file[i] = self.ts_file[i].replace(self.cam_name_no_space[i],
    ...
    def sync_setup(self):
        ...
        if len(self.cam) == 0:
            ...
            for i in range(len(self.cam)):
                ...
                self.base_name.append(self.cam_name_no_space[
        ...
        if self.running_config['debug_mode']:
            ...
            if len(self.cam) == 0:
                ...
                for i in range(len(self.cam)):
                    ...
                    self.vid_file.append(
                        os.path.normpath(self.dir_output.get() + '/' + self.base_name[i] + self.attempt.get() + '.avi'))
            ...
            try:
                if self.rows_fname_available:
                    ...
                    self.calibration_error = self.cgroup.calibrate_rows(all_rows, self.board_calibration,
                                                                        ...
                                                                        init_extrinsics=self.init_matrix, max_nfev=200,
                                                                        n_iters=6, n_samp_iter=200, n_samp_full=1000,
                                                                        verbose=True)
    ...
    def createGUI(self):
        ...
        for i in range(self.number_of_cams):
            ...
            self.camera_entry.append \
                (ttk.Combobox(init_camera_frame, textvariable=self.camera[i], width=10, justify="left"))
            ...
            self.format_entry.append \
                (ttk.Combobox(init_camera_frame, textvariable=self.formats[i], width=15, justify="left"))
            ...
            self.exposure_entry.append \
                (Entry(capture_settings_frame, textvariable=self.exposure[i], width=5, justify="left"))
            ...
            self.framerate_list.append \
                (ttk.Combobox(framerate_frame, textvariable=self.framerate[i], width=15, justify="left"))
            ...
            self.x_offset_spinbox.append \
                (Spinbox(partial_scan_frame, from_=0.0, to=100.0, increment=1, textvariable=self.x_offset_value[i], command=lambda index_cam=i, idx=i: self.set_x_offset(index_cam, idx), width=5))
            ...
            self.y_offset_spinbox.append \
                (Spinbox(partial_scan_frame, from_=0.0, to=100.0, increment=1, textvariable=self.y_offset_value[i], command=lambda index_cam=i, idx=i: self.set_y_offset(index_cam, idx), width=5))
        ...
        setup_video_label.grid(row=cur_ro w -1, column=1, padx=1, pady=1, sticky="nw")
        ...
        record_video_label.grid(row=cur_ro w -1, column=2, padx=1, pady=1, sticky="nw")
        ...
        self.button_on = Radiobutton(record_video_frame, text="Record On", selectcolor='green', indicatoron=0,
            ...
            grid \
            (sticky="nsew", row=0, column=0, padx=5, pady=3)
